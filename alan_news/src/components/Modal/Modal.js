...
const Modal = ({ isOpen, setIsOpen, showFeedback }) => 
{
	...
	const sendEmail = event => 
	{
		...
		emailjs.sendForm('gmail', 'alan_ai', event.target, 'user_dhVImkgxaL27bxQ8pLPQ5')
		.then((result) => 
		{ 
	...
	if(isOpen && showFeedback) 
	{
		body = (
			<div className = { classes.paper }>
				<Typography variant="h3">Something went wrong? Send us some feedback</Typography> <br />
				
				<form className={classes.form} onSubmit = { sendEmail }>
					<TextField name="name" label="Your name" variant="outlined" /><br />
					<TextField name="email" type="email" label="Your email" variant="outlined" /><br />
					<TextField 
						name="feedback"
						multiline rows={5} 
		...
		body = (
			<div className = { classes.paper }>
				...
				<div className = { classes.infoContainer }>
					...
					<div className = { classes.chipContainer }>
					{
						['CNN', 'Wired', 'BBC News', 'Time', 'IGN', 'Buzzfeed', 'ABC News'].map(source => 
